...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function export_results_denoising(matFilePath)
...
for muID = 1:muNum
    ...
    for imageID = 1:imageNum
        ...
        nexttile
        imshow(u_obsv_buf(:,:,:,imageID,shown_exNumID));
        title("observed")
...
for imageID = 1:imageNum
    ...
    histogram(Dx_org, nbins, 'BinLimits', BinLim, 'FaceColor', [0.5,0.5,0.5]);
    hold on
    histogram(Dx_opt(:,7), nbins, 'BinLimits', BinLim);
    ylim(ylimValue);
    xlabel(strcat(counters{7}, methodNameModified{7}), 'Interpreter','none');
    hold off
    ...
    histogram(Dx_org, nbins, 'BinLimits', BinLim, 'FaceColor', [0.5,0.5,0.5]);
    hold on
    histogram(Dx_opt(:,8), nbins, 'BinLimits', BinLim);
    ylim(ylimValue);
    xlabel(strcat(counters{8}, methodNameModified{8}), 'Interpreter','none');
    hold off
