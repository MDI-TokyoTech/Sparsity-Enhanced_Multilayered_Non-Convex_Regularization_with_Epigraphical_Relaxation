function [x, time_opt, convergeNum] = ERLiGME_DVTV_GPU(u_obsv, Phi, Phit, para, u_org, enhance, u_opt)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
% Psi(x) = Zero(u) + || z ||_1
% Psi(Lcx) = Psi(x) + ι_epi_L2(DCu,z) + iota_[0 1](u)
...
prox_psi2{3} = @(z1, z2) Proj_Epigraph_L2_for_DVTV_GPU(z1, z2, para.wlumi);
...
L = @(z) z;
Lt = @(z) z;
% Lc = @(z) { z{1}, z{2}, D(C(z{1})), z{2} };
% Lct = @(z) { z{1} + Ct(Dt(z{3})), z{2} + z{4} };
Lc = @(z) { z{1}, z{2}, D(C(z{1})), z{2}, z{1} };
Lct = @(z) { z{1} + Ct(Dt(z{3})) + z{5}, z{2} + z{4} };
...
if enhance
    ...
    z_star = func_L2_for_DVTV(gather(D(C(u_opt))), para.wlumi);
    ...
    x = cell(1, 2);
    x{1} = u_obsv;
    x{2} = 0*func_L2_for_DVTV(gather(D(C(u_opt))), para.wlumi);
...
% Step size
%-----------------------------------------------------
% (2-layer)L'L = [2I + (DC)'DC, O]
%                [O, 2I]
% ||L'L||_op <= || 2I + (DC)'DC ||_op
% max(eig(C'C)) = 1
% max(eig(D'D)) <= 8
% ||L'L||_op <= ( 2 + 1*8 ) = 10
LctLc_op = 10;
