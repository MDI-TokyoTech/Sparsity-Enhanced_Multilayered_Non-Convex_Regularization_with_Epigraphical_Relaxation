function [x, time_opt, convergeNum] = DVTV_GPU_vector(u_obsv, A, para, u_org)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% no-enhanced DVTV
...
% Lc = [DC;
%        I]
if exist(sprintf('output/%s', mfilename), 'dir') == 0
    mkdir(sprintf('output/%s', mfilename));
end

%-----------------------------------------------------
% Define L and Lc
%-----------------------------------------------------
if exist(sprintf('output/%s/Lmat_%03dx%03dimage.mat', mfilename, para.cols, para.rows), 'file') == 0
    ...
    save(sprintf('output/%s/Lmat_%03dx%03dimage.mat', mfilename, para.cols, para.rows),...
        "Lc_op2", "r", "Lmat", "Lcmat");
else
    load(sprintf('output/%s/Lmat_%03dx%03dimage.mat', mfilename, para.cols, para.rows));
end
