function [l, s, time_opt, convergeNum] = FRPCA_ERLiGME_rangeConstraint(u_obsv, para, u_org, enhance, rangeConstraint, u_opt)
%==================================
...
% Psi(Lc*u) = Psi(L*u) + iota_{epi_L2}(Tl, z) + iota_{||.||_1 <= lambda_2}(s) + iota_{epi_L2}(Tl, z)
...
Lc_op2 = 2 + 2; % constraint OFF (eigs(max(2I+(Tr'Tr + Ti'Ti)), 1))
...
tau = (para.kappa/2 + 2/para.kappa)*(theta/para.mu) + (para.kappa - 1); % for update of v
...
L = @(z) z;
Lt = @(z) z;
Lc = @(z) { z{1}, z{2}, z{3}, z{1}, z{2}, T(z{1}), z{3} };
Lct = @(z) { z{1} + z{4} + Tt(z{6}), z{2} + z{5}, z{3} + z{7} };
...
prox_psi2{4} = @(z, gamma) Proj_RangeConstraint(z, rangeConstraint);
prox_psi2{5} = @(z, gamma) ProjL1ball(z, zeros(size(z)), l2);
prox_psi2{6} = @(z1, z2, gamma) Proj_Epigraph_L2_for_ASNN(z1, z2, gamma);
% for color image
if para.dim > 1
...
for i = 1:para.maxiter
    ...
    [wtemp_1, wtemp_2] = prox_psi2{6}(w{6}, w{7}, 1);
    w{6} = w{6} - wtemp_1;
    w{7} = w{7} - wtemp_2;
