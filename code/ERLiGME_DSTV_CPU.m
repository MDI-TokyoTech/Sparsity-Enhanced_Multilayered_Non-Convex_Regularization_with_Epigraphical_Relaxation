function [x, time_opt, convergeNum] = ERLiGME_DSTV_CPU(u_obsv, Phi, Phit, para, u_org, enhance, u_opt)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Moreau-enhanced DSTV with Epigraph relaxation
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% input is vector, but process it as matrix.
rows = para.rows;
cols = para.cols;
dim = para.dim;
N = rows*cols*dim;
...
P = @(z) func_PeriodicExpansionGrad_notGPU(z, blocksize, shiftstep, isTF);
Pt = @(z) func_PeriodicExpansionTransGrad_notGPU(z, isTF);
switch kernel
    case 'Gaussian'
...
prox_psi1{2} = @(z, gamma) Prox_DVTVnorm_Jcb(z, gamma, para.wlumi);
% % proximity operator with constraint
prox_psi2 = cell(1,4);
prox_psi2{1} = @(z, gamma) z;
prox_psi2{2} = @(z, gamma) Prox_DVTVnorm_Jcb(z, gamma, para.wlumi);
prox_psi2{3} = @(z1, z2) Proj_Epigraph_cwSTV(z1, z2, blocksize);
...
L = @(z) z;
Lt = @(z) z;
% Lc = @(z) { z{1}, z{2}, K(P(D(C(z{1})))), z{2} }; % not [0,1] constraint
% Lct = @(z) { z{1} + Ct(Dt(Pt(Kt(z{3})))), z{2} + z{4} };
Lc = @(z) { z{1}, z{2}, K(P(D(C(z{1})))), z{2}, z{1} };
Lct = @(z) { z{1} + Ct(Dt(Pt(Kt(z{3})))) + z{5}, z{2} + z{4} };
...
if enhance
    ...
    z_star = func_NN_for_DSTV(gather(K(P(D(C(u_opt))))), blocksize);
    ...
    x = cell(1, 2);
    x{1} = u_obsv;
    x{2} = 0*func_NN_for_DSTV(gather(K(P(D(C(u_obsv))))), blocksize);

    A = @(z) {Phi(z{1})}; % A = [Phi O]
    At = @(z) {Phit(z{1}), zeros(size(x{2}))};
    B = @(z) {0*z{1}, 0*z{2}};
    Bt = @(z) {0*z{1}, 0*z{2}};
end
...
% Step size
%-----------------------------------------------------
% (2-layer)L'L = [2I + (KPDC)'KPDC, O]
%                [O, 2I]
% ||L'L||_op <= || 2I + (KPDC)'KPDC ||_op
%
% (3-layer)L'L = [2I + (KPDC)'KPDC, O, O]
%                [O, 2I, O]
%                [O, O, 3I]
% ||L'L||_op <= max( || 2I + (KPDC)'KPDC ||_op, 3 )
%
% max(eig(C'C)) = 1
% max(eig(P'P)) = 1
% max(eig(K'K)) = 1/prod(blocksize)
% max(eig(D'D)) < 8
% ||L'L||_op <= ( 2 + 1*1*8/prod(blocksize) ) = 2.8889
if isequal(blocksize, shiftstep)
    LctLc_op = 10;
else
