function [x, time_opt, convergeNum] = LiGME_DVTV_single_GPU(u_obsv, A, para, u_org, enhance)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Phi = @(z) {A*z{1}};
Phit = @(z) {A'*z{1}};
L = @(z) {full(Lmat)*z{1}}; % sparse * single is not allowed
Lt = @(z) {full(Lmat')*z{1}};
Lc = @(z) {full(Lmat)*z{1}, z{1}}; % add constraint here
Lct = @(z) {full(Lmat')*z{1} + z{2}};
