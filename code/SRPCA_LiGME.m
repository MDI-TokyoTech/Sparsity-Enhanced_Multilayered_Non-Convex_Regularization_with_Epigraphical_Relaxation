function [l, s, time_opt, convergeNum] = SRPCA_LiGME(u_obsv, para, u_org, enhance, rangeConstraint)
%==================================
...
%
% Psi(L*u) = lambda_1*|| l ||_* + zero(s)
% Psi(Lc*u) = Psi(L*u) + iota_{||.||_1 <= lambda_2}(s)
...
%  Psi(Lc*u) = Psi(L*u) + iota_{||.||_1 <= lambda_2}(s) + iota_{[a,b]}(l)
% Lc = [I O]
%      [O I]
%      [O I]
%      [I O]
...
At = @(z) {z{1}, z{1}};
if enhance
    B = @(z) {sqrt(theta/para.mu)*z{1}, 0*z{2}};
    Bt = @(z) {sqrt(theta/para.mu)*z{1}, 0*z{2}};
    B_op2 = theta/para.mu;
else
...
if ~exist('rangeConstraint', 'var')
    ...
    prox_psi2{3} = @(z, gamma) ProjL1ball(z, zeros(size(z)), l2);
    ...
    Lc = @(z) {z{1}, z{2}, z{2}, z{1}};
    Lct = @(z) {z{1}+z{4}, z{2}+z{3}};
    LctLc_op2 = 4;
end
