function [l, s, time_opt, convergeNum] = SRPCA_LiGME_onlyNN(u_obsv, para, u_org, enhance, rangeConstraint)
%==================================
%  input: 2D shifted signal
% output: Low-rank signal, Sparse noise
% 
% Warning: this setting results in non-convex optimization
%==================================
% u = [l, s]^T
% Phi = [I I]
% u* = argmin_u 1/2|| Phi*u - y ||_F^2 + mu*Psi(L*u)

% Psi(L*u) = lambda_1*|| l ||_*
...
%  Psi(Lc*u) = Psi(L*u) + iota_{[a,b]}(l) + iota_{||.||_1 <= lambda_2}(s)
% Lc = [I O]
%      [O I]
%      [I O]
%      [O I]
...
if ~exist('rangeConstraint', 'var')
    ...
    Lc = @(z) {z{1}, z{2}, z{1}, z{2}};
    Lct = @(z) {z{1}+z{3}, z{2}+z{4}};
    LctLc_op2 = 2;
end
