function [x, time_opt, convergeNum] = STV_GPU_vector(u_obsv, A, para, u_org)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% no-enhanced STV
...
cols = para.cols;
dim = para.dim;
if exist(sprintf('output/%s/Lmat_%03dx%03dimage_bsize(%dx%d)_shift(%d.%d).mat',...
    ...
    save(sprintf('output/%s/Lmat_%03dx%03dimage_bsize(%dx%d)_shift(%d.%d).mat',...
        mfilename, para.cols, para.rows, para.blocksize(1), para.blocksize(2),...
        para.shiftstep(1), para.shiftstep(2)),...
        "Lc_op2", "r", "Lmat", "Lcmat");
else
    load(sprintf('output/%s/Lmat_%03dx%03dimage_bsize(%dx%d)_shift(%d.%d).mat',...
        mfilename, para.cols, para.rows, para.blocksize(1), para.blocksize(2),...
        para.shiftstep(1), para.shiftstep(2)));
end
